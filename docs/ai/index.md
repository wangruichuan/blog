...
符号主义认为，世间万物都能找到背后的规律。

但不断的研究发现，这条路走到头啦，很多问题，人类实在想不到怎么写成一个明确的函数，从上帝视角来看，人类还是太菜了。


现代人工智能的思路：**猜和简化问题**

找不到精确的函数，那就找一个从结果上看大差不差的函数，然后连蒙带猜，逐渐逼近真实答案就好了
...
我们知道，可以用一个线性函数去拟合一条直线，但

怎么把线性函数变成非线性函数，很简单，在这个函数的最外层，再套一个非线性的运算就可以了。比如 平方，sin，e... 

这就是**激活函数**

一开始，只有一个输入变量就是X，如果有多个输入变量，那每一个x都要对应一个w

有时，只套一层激活函数没有很好的起到效果，曲线弯的还不够灵活。

很简单，只需要将之前的作为一个整体，再进行一次线性变换，然后再套上一个激活函数，这样理论上就可以逼近任意的连续函数。

#### 神经网络

![](https://pic1.imgdb.cn/item/68b7ae8858cb8da5c874c18f.png)

上图也就是所谓的 神经网络。

重新再看一下这个过程，从神经网络的这个图来看的话，似乎就像是一个信号从左到右传播了过去，那这个过程就叫做神经网络的**前向传播**


但实际上，就是一点点的分步骤把一个函数的值计算出来了而已。

虽然最终的函数可能非常复杂，但是我们的目标非常的简单和明确。

就是**根据已知的一组 x，y值，猜出这里所有的w和b分别是什么**。
...
**损失函数**：表示预测数据与真实数据误差的函数

常用的是 均方误差（MSE)

![](https://pic1.imgdb.cn/item/68b7b14758cb8da5c874d367.png)


最终这个也可以表示成一个 包含w和b的函数

我们的最初目的是求出 **让损失函数的值最小的w和b**，怎么求解呢？

很简单，导数等于0，求极值点。

对于这种多元函数求极值的问题，要让每个参数的偏导数等于0即可。


#### 梯度下降

回到神经网络，神经网络是一个通过线性函数和非线性激活函数不断组合形成的一个非常复杂的非线性函数，那它对应的损失函数是一个更复杂的非线性函数，往往不能直接通过求导获得极值。

那怎么办呢，人们的解决办法非常简单粗暴，就是一点点试。

每次把参数向着**让损失函数变小的那个方向调整一点点**，直到让损失函数足够小。

具体怎么调呢？

回到最初始的状态，w变化一点点，使得损失函数会变化多少，这其实就是损失函数对w的偏导数，对b来说同样如此。

而我们要做的，就是让w和b不断往偏导数的反方向去变化，那具体增加的快慢，再增加一个系数去控制，我们叫其 **学习率**。

这些偏导数所构成的向量，就是**梯度**，而不断变化的w和b，让损失函数逐渐减小的一个过程，进而求出最终的w和b，这个过程就叫做**梯度下降**

![](https://pic1.imgdb.cn/item/68b7c11758cb8da5c875762b.png)



#### 反向传播

关键在于，偏导数怎么求：可以使用链式法则


![](https://pic1.imgdb.cn/item/68b7c29858cb8da5c87583c8.png)

由于我们可以从右往左依次求导，然后逐步更新每一层的参数，直到我们把所有的神经网络的参数都更新一遍。

在计算前一层时，用到的一些偏导数值后面也会用到（比如 l对w1的偏导 和 l对w2的偏导 都需要用到 l对^y的偏导），所以说不用计算那么多，而是让这些值从右到左一点点传播过来就好，这样一个过程叫**反向传播**。


我们通过前向传播，根据输入x计算出y，然后再根据反向传播，计算出损失函数关于每个参数的梯度，然后每个参数都向着梯度的反方向变化一点点，这就构成了神经网络的一次训练，而神经网络经过多轮的训练，让参数一点点的变化，直到让损失函数足够小

#### 过拟合

![](https://pic1.imgdb.cn/item/68b7c60c58cb8da5c8759d15.png)

神经网络不是越大越好，出现过拟合时，简化模型是一个方面，与之相对的是增加训练数据的量，只要数据的量足够充足，原本复杂的模型就相对变得简单了。

那有的时候我们懒得去收集更多的数据，那我们可以通过已有的数据去创造数据，比如图像里，你可以旋转，镜像，裁剪，噪声等操作，创造出更多的训练样本，这就叫做**训练增强**。

那是否可以通过干预训练过程去入手，阻止过拟合的发生呢？

其实训练过程就是不断调整参数的过程，那其实只要不要让参数继续向着过拟合的方向发展就可以了，那最简单的方法就是提前终止训练过程。

> 能否直接抑制参数的野蛮生长呢？

可以在损失函数中，把参数本身的值加上去，这样在参数往大了调整时，如果让损失函数减小的没有那么多，导致新的损失函数反而是变大的，那么此时调整就是不合适的。

因此一定程度上就抑制了参数的野蛮增长。


![](https://pic1.imgdb.cn/item/68b7ca0558cb8da5c875a2d3.png)

我们把这一项叫 惩罚项，把通过这种向损失函数中添加权重的方法叫 **正则化**

这些控制参数的参数，我们以后统称为叫 **超参数**

还有一种方法叫dropout，可以在训练过程中，每次都随机丢弃一部分参数，从而避免了在某些关键参数上过度依赖的风险。


#### 其他问题

![](https://pic1.imgdb.cn/item/68b7cb7e58cb8da5c875ab5c.png)
...
和经典的神经网络相比就是多了一个隐藏状态

![](https://pic1.imgdb.cn/item/68b7e18458cb8da5c876810c.png)
...
https://www.bilibili.com/video/BV1C3dqYxE3q
...
把位置编码加到原来的词向量中（直接加！），现在这个词就有了位置信息。

但此时，每个词还没有其它词的上下文信息，也就是注意不到其他词的存在.

接下来需要这样做：

1. 用一个wq矩阵（通过训练过程学习得到的一组权重值）和第一个词向量做相乘，得到维度不变的q1向量
2. 用一个wk矩阵和第一个词向量做相乘，得到维度不变的k1向量
3. 用一个wv矩阵和第一个词向量做相乘，得到维度不变的v1向量
4. 接着对其他词向量，也和相同的WQKV矩阵相乘，分别得到自己对应的QKV向量
...
q1 * k2 : 在第一个词的视角里，第一个词和第二个词的相似度是多少  ，得到 a12

计算出 a11（和自己做点积）， a12，a13，a14，然后分别和v向量相乘再相加得到 **a1**

![](https://pic1.imgdb.cn/item/68b7e7ee58cb8da5c8769202.png)


此时，这个词就表示：在第一个词的视角下，按照和它相似度大小，按权重，把每个词的词向量都加到了一块，这就把全部上下文的信息都包含在第一个词当中了，而且是用第一个词的视角来看的。

同理，其他几个词也都按照这种方式，那么此时，每个词都把其他词的词向量按照和自己的相似度权重，加到了自己的词向量中

![](https://pic1.imgdb.cn/item/68b7e8ee58cb8da5c876931a.png)


我们从全局视角来看，现在就是把最初的输入的词向量，经过一番处理，变成了一组新的词向量，不一样的是，这组新的词向量中，每一个都是包含了位置信息和其他词上下文信息的一组新的词向量，这就是 注意力机制attention 做的事情。

#### 多头注意力机制

有时候，一个词和另一个词的关系，可能从不同的视角看 是不一样的，对于注意力机制来说，如果只通过一种方式计算一次相关性，灵活性就会大大降低。

之前我们是每个词计算一组QKV,现在我们在QKV的基础上，再经过两个权重矩阵变成两组QKV，给每个词两个学习机会。学习到不同的要计算相似度的QKV，来增加语言的灵活性，这里的每组QKV成为一个头


![](https://pic1.imgdb.cn/item/68b7ef8558cb8da5c876a173.png)

接下来，在每个头的QKV，仍然经过刚刚的注意力层的运算，得到a向量，然后把两个a向量拼接起来，得到了和刚刚一样的结构，这种是两头的情况，也就叫做 多注意力机制。


---
GPT的底层其实就只有 transformer 的一半，只有解码器的部分，也不翻译谁，只管看前面的词猜下一个词。

比如给定n个词，预测第n+1个词，transformer的自注意力机制他是**默认每个词都能看到其他位置的词的**。这对翻译任务场景来说，可能是没有问题的，但对于生成任务，就会有很大的问题。


比如，你想生成一句话，“我喜欢看电影”，当模型生成“我喜欢”的时候，它就不应该看到 看和电影，怎么做到让模型不去看未来呢？-----掩码注意力
...
自注意力机制让每个词元在上下文中“看别人”，整合其他词的信息。

但仅靠这些信息还不够，Transformer 还需要进一步让每个词自己思考、提炼、变换。这个任务，就交给了前馈神经网络层（Feed Forward Neural Network，简称 FFN）。
...
<span style="text-decoration: underline;">如何消化？</span>
...
模型还需要进一步处理它，让它具备更强的语义表达能力和推理能力。这时，“它”的向量就会被送入前馈网络中：

- 前馈层会先把这个向量升维到一个更高维度（比如从 768 维扩展到 3072 维）；
- 接着，经过 GELU 激活函数进行非线性处理；
- 然后再降维回原来的大小，形成一个新的、更复杂的向量表示。

这个新表示就是“它”这个词元的升级版本，里面包含了模型对“它”这个词更深层的理解，可以帮助模型提取出更适合判断行为、情感、逻辑的语义特征。

最终形成的向量将送入下一层 Transformer，继续处理。

因此，前馈层也是非常重要的一层，看起来只是“对每个词做了一遍变换”，但它非常关键。


如果 Transformer 只有注意力层没有前馈层，那它就像是一个只会模仿别人的人，不会自己总结、推理和表达。如果说自注意力阶段是“看外部世界”，那么前馈网络阶段就是“闭上眼睛思考自己”。
...
- 意义：快速的检索
- 向量数据库本身不生成向量，向量是由embedding模型生成的
- 与传统数据库的关系是互补关系
...
模型在某些方面不够，所以我们通过一些训练的方法，对模型进行一个更新，那更新后的模型，在某一个方面上希望他的能力会提升，本质上是对模型的一个改动。

模型的改动，本质上是对模型参数的改动。原有的参数 + 改动的量 = 新的参数。

归根结底，如何获得这个 改动的量。

第一种方法：这里的每个参数我都要通过学习的方法把它得出来，这种叫做 全量的微调

第二种方法：用更少的资源进行微调，这种叫PEFT，其中最常见的叫LoRA。
...
> 
> Pretraining : 让小孩自学，好的坏的都去看，对事物形成基本的理解，但可以为下一步找老师做准备了。
> 
> SFT : 有一个老师，精挑细选的教学生知识。因为小孩之前已经有了足够的积累，在老师的这样一个指导下，很快就能举一反三，在相对较少的一个训练下，小孩就能出师了。类似chatgpt这种生成式的模型，是以这个回答的形式，以prompt response 来处理模型。但从广义上来讲，sft阶段可以根据任务的不同来实现不同的方式：比如说 机器翻译、语义分析，那SFT就可以有不同的方法了。
> 
> RLHF : 高人指点。不停的给小孩反馈，你的答案是 好还是不好，小孩根据高人的回答，不断的调整自己，让自己的回答越来越倾向于高人的这个认可，最后小孩这个回答的答案就水平越来越高，最后就出师了。


核心思想：通过对大模型进行一些训练，对它的一些参数再次重新调整，然后让它掌握到这样的一些知识，所以本质上，我们对这个大模型微调之后，它就产生了一个新的模型，那接下来用户再有问题，就直接问这个新的模型。这个高人本身也是训练出来的，是一个 reward model。通过人类的一个排序，人这个reward model 学习到 人类喜欢的

### 为什么需要 微调？

### 什么场景下用 微调？

- 对话风格微调：模型的语气和回答方式，使其更符合客服、虚拟助理等场景的需求
- 知识灌注：快速集成领域知识，使得模型更准确理解专有术语和规则，提高专业问答能力
- 推理能力提升：优化模型对长文本的理解和推理能力


### SFT 监督微调

Supervised Fine-Tuning , 对已训练的模型进行特定任务的训练，以提供他们在该任务上的表现

**用带标签的样本（输入-输出对）来微调一个预训练好的大模型**，让模型学会按照人类期望的方式回答问题或完成任务。


你有一个预训练的大语言模型（比如 LLaMA），你给它一堆“问题 + 标准答案”的数据（比如问答对），然后训练它“看到问题后输出标准答案”。这就是 SFT。

👉 SFT 关注的是：用什么数据、什么目标来训练（有监督的指令数据）。

优点：
1. 提升性能。在特定任务的数据上进行微调，模型可以显著的提高在该任务上的表现，模型能学习到与任务有关的更多的特征表达
2. 减少标注数据的需求
3. 灵活性
4. 个性化适应：领域内微调

缺点：
1. 过拟合的风险：模型在训练数据上表现良好，在测试数据上表现不佳
2. 仍然需要一定的计算资源
3. 泛化能力：过度微调可能会导致其他任务的泛化能力下降

### LoRA 

> 什么是全量微调？
> 全量微调（Full Fine-Tuning） 是指：在微调阶段，更新模型的所有参数。
> 全量微调关注的是：训练时更新哪些参数。
> ![](https://pic1.imgdb.cn/item/68bf647d58cb8da5c88afdff.png)
> ❗ SFT ≠ 全量微调
> ✅ SFT 可以搭配全量微调，也可以搭配参数高效微调方法（如 LoRA）


核心背景：

举例说明：假如我们给张三提了一个任务，让他编写一个文章。


## RAG


怎么让大模型学习到一些它以前没有掌握的知识呢？

核心思想：很朴素，在大模型外面单独去准备一个知识库，用户有问题，直接先从这个知识库里面检索，检索到相关的知识以后，再把这些知识给到大模型进行一个学习加工，那大模型就基于用于的这个问题去整理答案去给到我们的用户。
...
- query改写和语义理解
  - 好吃的川菜 → 自动扩展成 “麻辣火锅” “水煮鱼”
- 意图识别：快速识别用于是想订餐还是查菜谱
- 生成embedding用于召回匹配
  - 在海量数据中，找到相关内容